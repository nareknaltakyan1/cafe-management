...
public class OrderService {
    ...
    public ProductInOrderDTO addProductToOrder(ProductInOrderDTO productInOrderDTO) {
        ...
        if (productInOrderOpt.isPresent()) {
            final String errorMsg = MessageFormat.format("Another Product in Order with orderId {0} and productId {1} already exists", orderId, productId);
            log.error(errorMsg);
    ...
    public Optional<ProductInOrderDTO> getProductInOrder(Long orderId, Long productId) {
        ...
        return productInOrderRepository.findByOrderIdAndProductId(orderId, productId)
                .map(this::productInOrderEntityToDto);
    }
